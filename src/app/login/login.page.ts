...
export class LoginPage implements OnInit {
  ...
  async loginGoogleAndroid() {
    try {
      const res = await this.googlePlus.login({
        webClientId:
          '71405197754-h569scocea9q3s9mk34spsugr3e8ko20.apps.googleusercontent.com',
        offline: true,
      });
      ...
      if (user && user.email.endsWith('correounivalle.edu.co')) {
        const users: Google = {
          uid: user.uid,
          email: user.email,
        ...
        await this.firestoreService.creatDoc(users, 'Users', users.uid);
        alert('Ingresando...');
        ...
        const alert = await this.alertController.create({
          header: 'Error',
          message:
            'Solo se permiten usuarios con correo de correounivalle.edu.co',
          buttons: ['OK'],
        });
        await alert.present();
        res.idToken = null;
  ...
  async loginGoogleWeb() {
    try {
      ...
      if (
        currentUser &&
        (await currentUser).email.endsWith('correounivalle.edu.co')
      ) {
        const user: Google = {
  ...
  async iniciarSesion() {
    if (!this.email || !this.password) {
      ...
      const loggedIn = await this.firebaseauthService.login(
        this.email,
        this.password
      );
      if (loggedIn) {
